...
class CoGanh:
    ...
    def can_not_move(self, board, position):
        ...
        # Check có thể di chuyển lên trên, dưới, trái phải không?
        if x > 0:
            if board[x - 1][y] == 0:
        ...
        if x > 0:
            if board[x - 1][y] == player:
                result = result and self.can_not_move(board, (x - 1, y))
        if y > 0:
            if board[x][y - 1] == player:
                result = result and self.can_not_move(board, (x, y - 1))
        if x < 4:
            if board[x + 1][y] == player:
                result = result and self.can_not_move(board, (x + 1, y))
        if y < 4:
    ...
    def chan(self, board, player):
        ...
        pos = self.get_position(board, player)

        # Check which position is "chan"ed
        for p in pos:
            if self.can_not_move(board, p):
    ...
    def random_move(self, node, player):
        pos = self.get_position(node.board, player)
        possible_moves = []
